...
defmodule ExGoCD.ConfigRepos do
  ...
  def refresh_config_repo(config_repo) do
    # Delegate to the Poller — now a Horde singleton via DistSingleton.
    case ExGoCD.DistSingleton.whereis(ExGoCD.ConfigRepos.Poller) do
      nil ->
  ...
  def refresh_config_repo_with_content(config_repo, content) when is_binary(content) do
    case Parser.parse_and_upsert(content, config_repo.id) do
      {:ok, count} ->
