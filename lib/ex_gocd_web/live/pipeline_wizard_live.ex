defmodule ExGoCDWeb.PipelineWizardLive do
  ...
  def mount(params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:connection_status, nil) # nil, :checking, :success, :failed
        ...
        <div class="bg-white border border-[#d6e0e2] rounded shadow-sm overflow-hidden">
          <div class="bg-[#e7eef0] px-6 py-3 border-b border-[#d6e0e2]">
            <h2 class="text-xs font-bold uppercase tracking-wider text-slate-700">
              <%= case @step do %>
                <% 1 -> %>Pipeline Group &amp; Name
                <% 2 -> %>Configure Material (Source Control)
                <% 3 -> %>Define Stage
                <% 4 -> %>Configure Job &amp; Task
              <% end %>
            </h2>
          </div>
            ...
            <form phx-submit="next_step" phx-change="validate_form">
              <%= case @step do %>
                <% 1 -> %>
                  <div class="space-y-4">
                    <div>
                      <label class="block text-xs font-bold text-slate-600 mb-1">Pipeline Name <span class="text-rose-500">*</span></label>
                      <input type="text" name="name" value={@form["name"]} placeholder="e.g. build-service" required
                    ...
                    <div>
                      <label class="block text-xs font-bold text-slate-600 mb-1">Pipeline Group <span class="text-rose-500">*</span></label>
                      <div class="relative max-w-lg">
                        <input type="text" name="group" value={@form["group"]} placeholder="e.g. defaultGroup" required list="pipeline-groups-list"
                ...
                <% 2 -> %>
                  <div class="space-y-4">
                    <div>
                      ...
                      <select name="material_type" class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]">
                        <option value="git" selected={@form["material_type"] == "git"}>Git</option>
                        <option value="svn" selected={@form["material_type"] == "svn"}>Subversion</option>
                        <option value="hg" selected={@form["material_type"] == "hg"}>Mercurial</option>
                      </select>
                    </div>

                    <div>
                      <label class="block text-xs font-bold text-slate-600 mb-1">Repository URL <span class="text-rose-500">*</span></label>
                      <input type="text" name="material_url" value={@form["material_url"]} placeholder="e.g. https://github.com/example/repo.git" required
                             class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                      <%= if @errors["material_url"] do %>
                        <p class="text-[11px] text-rose-500 mt-1 font-semibold">{@errors["material_url"]}</p>
                      <% end %>
                    </div>

                    <div>
                      <label class="block text-xs font-bold text-slate-600 mb-1">Branch</label>
                      <input type="text" name="material_branch" value={@form["material_branch"]} placeholder="e.g. master"
                             class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 max-w-lg">
                      <div>
                        ...
                        <input type="text" name="material_username" value={@form["material_username"]}
                               class="w-full px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                      </div>
                      <div>
                        <label class="block text-xs font-bold text-slate-600 mb-1">Password</label>
                        <input type="password" name="material_password" value={@form["material_password"]}
                               class="w-full px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                      </div>
                    </div>

                    <div class="pt-2 flex items-center gap-3">
                      <button type="button" phx-click="check_connection" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-350 text-slate-700 rounded text-xs font-semibold flex items-center gap-1.5 transition-all">
                      ...
                      <%= if @connection_status == :failed do %>
                        <span class="text-xs text-rose-500 font-bold flex items-center gap-1">
                          <i class="fa fa-circle-exclamation"></i> Connection failed (invalid URL or credentials)
                        </span>
                      <% end %>
                    </div>
                  </div>
                ...
                <% 3 -> %>
                  <div class="space-y-4">
                    <div>
                      ...
                      <input type="text" name="stage_name" value={@form["stage_name"]} placeholder="e.g. build-stage" required
                             class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                      <%= if @errors["stage_name"] do %>
                        <p class="text-[11px] text-rose-500 mt-1 font-semibold">{@errors["stage_name"]}</p>
                      <% end %>
                    </div>
                    ...
                    <div>
                      ...
                      <div class="space-y-2 text-xs">
                        <label class="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                          <input type="radio" name="approval_type" value="success" checked={@form["approval_type"] == "success"}
                                 class="radio radio-xs checked:bg-[#943a9e] border-slate-350" />
                          On Success (automatic)
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                          <input type="radio" name="approval_type" value="manual" checked={@form["approval_type"] == "manual"}
                                 class="radio radio-xs checked:bg-[#943a9e] border-slate-350" />
                          Manual Trigger
                        </label>
                      </div>
                ...
                <% 4 -> %>
                  <div class="space-y-4">
                    <div>
                      ...
                      <input type="text" name="job_name" value={@form["job_name"]} placeholder="e.g. build-job" required
                             class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                      <%= if @errors["job_name"] do %>
                        <p class="text-[11px] text-rose-500 mt-1 font-semibold">{@errors["job_name"]}</p>
                      <% end %>
                    </div>
                    ...
                    <div class="border-t border-[#e9edef] my-4 pt-4">
                      ...
                      <div class="space-y-4">
                        <div>
                          ...
                          <select name="task_type" class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]">
                            <option value="exec" selected={@form["task_type"] == "exec"}>Exec Task (Run command)</option>
                        ...
                        <div>
                          <label class="block text-xs font-bold text-slate-600 mb-1">Command <span class="text-rose-500">*</span></label>
                          <input type="text" name="task_command" value={@form["task_command"]} placeholder="e.g. mix, make, npm, echo" required
                                 class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]" />
                          <%= if @errors["task_command"] do %>
                            <p class="text-[11px] text-rose-500 mt-1 font-semibold">{@errors["task_command"]}</p>
                          <% end %>
                        </div>

                        <div>
                          <label class="block text-xs font-bold text-slate-600 mb-1">Arguments</label>
                          <textarea name="task_arguments" placeholder="e.g. test&#10;compile (one per line)" rows="3"
                                    class="w-full max-w-lg px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e] font-mono">{@form["task_arguments"]}</textarea>
                          <p class="text-[10px] text-slate-400 mt-0.5">Enter arguments, one per line.</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="mt-8 pt-4 border-t border-[#e9edef] flex justify-between">
                <div>
                  <%= if @step > 1 do %>
                    <button type="button" phx-click="prev_step" class="px-4 py-2 rounded bg-white border border-slate-350 hover:bg-slate-50 text-slate-700 text-xs font-semibold transition-all">
                      <i class="fa fa-chevron-left mr-1"></i> Previous
                    </button>
                  <% end %>
                </div>
                <div class="flex gap-2">
                  <a href="/admin/pipelines" class="px-4 py-2 rounded bg-white border border-slate-350 hover:bg-slate-50 text-slate-600 text-xs font-semibold flex items-center transition-all">
                    Cancel
                  </a>
                  <%= if @step < 4 do %>
  ...
  def handle_info(:complete_connection_check, socket) do
    ...
    status = if String.starts_with?(url, "http") or String.starts_with?(url, "git@"), do: :success, else: :failed
      ...
      if name == "" do
        ...
        cond do
          !Regex.match?(~r/^[a-zA-Z0-9_\-\.]+$/, name) ->
            Map.put(errors, "name", "Name must contain only letters, numbers, hyphens, underscores, and periods")
