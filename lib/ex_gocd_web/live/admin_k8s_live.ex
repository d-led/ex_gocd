defmodule ExGoCDWeb.AdminK8sLive do
              ...
              <div class="text-sm text-gray-600 space-y-1">
                ...
                <%= if ElasticAgentProfile.resource_images(profile) do %>
                  <div class="mt-2 pt-2 border-t border-gray-100">
                    <span class="font-medium text-[11px] uppercase tracking-wide text-gray-500">
                      Resource → Image Mapping:
                    </span>
                    <div class="mt-1 space-y-0.5">
                      <%= for {resource, img} <- ElasticAgentProfile.resource_images(profile) |> Enum.sort() do %>
                        <div class="flex items-center gap-1 text-[10px] font-mono">
                          <span class="bg-gray-200 text-gray-700 px-1.5 py-0.5 rounded font-bold">
                            {resource}
                          </span>
  ...
  defp plugin_label("cd.go.contrib.elastic-agent.docker"), do: "docker-agent"
  defp plugin_label("ex_gocd.elasticagent.kubernetes"), do: "k8s-agent"
  defp plugin_label(unknown), do: String.slice(unknown, 0, 20)
end
