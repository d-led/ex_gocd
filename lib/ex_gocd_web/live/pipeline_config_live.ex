defmodule ExGoCDWeb.PipelineConfigLive do
  ...
  def mount(params, _session, socket) do
    ...
    if is_nil(pipeline) do
      ...
      {:ok,
       ...
       # :add_stage, :add_job, :add_task, :edit_task, :edit_material
  ...
  def handle_params(params, _url, socket) do
    ...
    pipeline_with_materials =
      if use_mock?(), do: pipeline, else: Repo.preload(pipeline, :materials)
      ...
      <div class="bg-white border-b border-[#e9edef] px-6 py-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <div class="flex gap-2">
          <a
            ...
            class="px-3 py-1.5 bg-white border border-slate-350 text-slate-700 rounded text-xs font-semibold hover:bg-slate-50 transition-all"
          >
            Back to pipelines
          </a>
        </div>
      ...
      <div class="max-w-[1400px] mx-auto px-6 py-6 flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-64 shrink-0 bg-white border border-[#d6e0e2] rounded shadow-sm overflow-hidden h-fit">
          ...
          <nav class="divide-y divide-[#e9edef] text-xs">
            <.nav_sidebar_link
              active={@view_mode == :general}
              href={"/go/admin/pipelines/#{@pipeline.name}/edit/general"}
            >
              <i class="fa fa-sliders mr-2"></i> General Settings
            </.nav_sidebar_link>
            <.nav_sidebar_link
              active={@view_mode == :materials}
              href={"/go/admin/pipelines/#{@pipeline.name}/edit/materials"}
            >
              <i class="fa fa-git-alt mr-2"></i> Materials ({@pipeline.materials |> length()})
            </.nav_sidebar_link>
            <.nav_sidebar_link
              active={
                @view_mode in [:stages, :stage_settings, :stage_jobs, :job_settings, :job_tasks]
              }
              href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages"}
            >
                ...
                <%= for s <- @pipeline.stages || [] do %>
                  <li class="space-y-1">
                    <div class="flex items-center justify-between">
                      <a
                        href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages/#{s.name}/settings"}
                        class={[
                          "hover:text-[#943a9e] flex items-center font-medium",
                          if(@active_stage && @active_stage.id == s.id,
                            do: "text-[#943a9e] font-bold",
                            else: "text-slate-600"
                          )
                        ]}
                      >
                      ...
                      <a
                        href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages/#{s.name}/jobs"}
                        class="text-[10px] text-[#943a9e] hover:underline"
                      >
                        Jobs
                      </a>
                    </div>
                      ...
                      <ul class="pl-4 border-l border-slate-200 ml-2 space-y-1.5 mt-1">
                        <%= for j <- s.jobs || [] do %>
                          <li>
                            <div class="flex items-center justify-between">
                              <a
                                href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages/#{s.name}/jobs/#{j.name}/settings"}
                                class={[
                                  "hover:text-[#943a9e] flex items-center",
                                  if(@active_job && @active_job.id == j.id,
                                    do: "text-[#943a9e] font-bold",
                                    else: "text-slate-500"
                                  )
                                ]}
                              ...
                              <a
                                href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages/#{s.name}/jobs/#{j.name}/tasks"}
        ...
        <div class="flex-grow bg-white border border-[#d6e0e2] rounded shadow-sm p-6">
          <%= if @flash_info do %>
            <div
              class="mb-5 bg-[#dbf1d9] border border-[#a3d7a8] text-[#298a4c] px-4 py-3 rounded flex justify-between items-center text-sm"
              role="alert"
            >
  ...
  defp general_settings_panel(assigns) do
    ~H"""
    <div>
      <h2 class="text-sm font-bold text-slate-700 border-b border-[#e9edef] pb-3 mb-5">
        General Settings
      </h2>
        ...
        <div class="space-y-4 max-w-xl">
          <div>
            ...
            <input
              ...
              value={@pipeline.name}
              disabled
              class="w-full px-3 py-2 rounded bg-slate-50 border border-[#d6e0e2] text-xs text-slate-500 cursor-not-allowed"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              name="label_template"
              value={@pipeline.label_template}
              required
              class="w-full px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]"
            />
            <p class="text-[10px] text-slate-400 mt-0.5">
              Use ${COUNT} as placeholder for run counter.
            </p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 rounded bg-white border border-[#d6e0e2] text-xs focus:outline-none focus:border-[#943a9e]"
            >
              <option value="none" selected={@pipeline.lock_behavior == "none"}>
                Unlock when finished / None
              </option>
              <option value="lockOnFailure" selected={@pipeline.lock_behavior == "lockOnFailure"}>
                Lock on failure
              </option>
              <option
                value="unlockWhenFinished"
                selected={@pipeline.lock_behavior == "unlockWhenFinished"}
              >
                Lock always / unlock when finished
              </option>
            </select>
          </div>
            ...
            <%= for material <- @pipeline.materials || [] do %>
              <tr class="hover:bg-slate-50/50">
                ...
                <td class="px-4 py-3 text-right">
                  <button
                    phx-click="open_edit_material"
                    phx-value-id={material.id}
                    class="text-[#943a9e] hover:underline font-bold mr-3"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    phx-value-id={material.id}
                    data-confirm="Are you sure you want to remove this material?"
              ...
              <tr class="hover:bg-slate-50/50">
                ...
                <td class="px-4 py-3 text-right">
                  ...
                  <button
                    ...
                    phx-value-id={s.id}
                    data-confirm="Are you sure you want to delete this stage?"
                    class="text-rose-500 hover:underline"
                  >
                    Delete
                  </button>
            ...
            <div class="space-y-2 text-xs">
              <label class="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                <input
                  ...
                  name="approval_type"
                  value="success"
                  checked={@stage.approval_type == "success"}
                  class="radio radio-xs checked:bg-[#943a9e]"
              ...
              <label class="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                <input
                  ...
                  name="approval_type"
                  value="manual"
                  checked={@stage.approval_type == "manual"}
                  class="radio radio-xs checked:bg-[#943a9e]"
                /> Manual Trigger
              </label>
            </div>
          ...
          <tbody class="divide-y divide-[#e9edef] bg-white">
            <%= for j <- @stage.jobs || [] do %>
              <tr class="hover:bg-slate-50/50">
                <td class="px-4 py-3 font-semibold text-slate-700">
                  <a
                    href={"/go/admin/pipelines/#{@pipeline.name}/edit/stages/#{@stage.name}/jobs/#{j.name}/settings"}
                    class="text-[#943a9e] hover:underline"
                  >
                    {j.name}
                  </a>
                  ...
                  <% else %>
                    <div class="flex gap-1">
                      <%= for r <- j.resources || [] do %>
                        <span class="bg-slate-100 border border-[#d6e0e2] px-1.5 py-0.5 rounded text-[10px] text-slate-600">
                          {r}
                        </span>
                      <% end %>
                    </div>
        ...
        <div class="space-y-4 max-w-xl">
          ...
          <div>
            ...
            <label class="flex items-center gap-2 cursor-pointer text-xs font-medium text-slate-700">
              <input
                type="checkbox"
                name="run_on_all_agents"
                value="true"
                checked={@job.run_on_all_agents}
              ...
              <tr class="hover:bg-slate-50/50">
                ...
                <td class="px-4 py-3 text-right">
                  ...
                  <button
                    ...
                    phx-value-id={task.id}
                    class="text-rose-500 hover:underline mr-3"
                  >
                    Delete
                  </button>
                  ...
                  <%= if index > 0 do %>
                    <button
                      ...
                      phx-value-id={task.id}
                      phx-value-dir="up"
                      class="p-0.5 border border-slate-200 hover:bg-slate-100 rounded text-[10px] text-slate-500 w-5 h-5 inline-flex items-center justify-center"
                    >
                  ...
                  <%= if index < length(@job.tasks) - 1 do %>
                    <button
                      ...
                      phx-value-id={task.id}
                      phx-value-dir="down"
                      class="p-0.5 border border-slate-200 hover:bg-slate-100 rounded text-[10px] text-slate-500 w-5 h-5 inline-flex items-center justify-center ml-1"
                    >
        ...
        <div class="bg-[#e7eef0] border-b border-[#d6e0e2] px-5 py-3 flex justify-between items-center">
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">
            <%= case @type do %>
              ...
              <% :add_material -> %>
                Add Repository Material
              <% :edit_material -> %>
                Edit Repository Material
            <% end %>
          </h3>
        ...
        <form phx-submit="save_modal">
              ...
              <% :add_stage -> %>
                <div>
                  ...
                  <input
                    ...
                    name="name"
                    required
                    class="w-full px-3 py-2 border border-[#d6e0e2] rounded bg-white text-xs"
                  />
                </div>
                <div>
              ...
              <% type when type in [:add_task, :edit_task] -> %>
                ...
                <div>
                  ...
                  <textarea
                    name="arguments"
                    rows="3"
                    class="w-full px-3 py-2 border border-[#d6e0e2] rounded bg-white text-xs font-mono"
                  >{@form["arguments"]}</textarea>
          ...
          <div class="bg-[#f4f8f9] border-t border-[#d6e0e2] px-5 py-3 flex justify-end gap-2">
            <button
              ...
              class="px-4 py-2 border border-slate-350 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold rounded"
            >
              Cancel
            </button>
            <button
              type="submit"
              class="px-4 py-2 bg-[#943a9e] hover:bg-purple-700 text-white text-xs font-semibold rounded border border-purple-700 shadow-sm"
            >
              Save Configuration
            </button>
          </div>
  ...
  defp nav_sidebar_link(assigns) do
    ...
    <a
      href={@href}
      class={[
        "block px-4 py-2.5 font-medium border-l-2 hover:bg-slate-50 transition-all",
        if(@active,
          do: "border-[#943a9e] text-[#943a9e] bg-purple-50/20 font-bold",
    ...
    case Pipelines.update_job(job, %{
      ...
      {:ok, _updated} ->
        ...
        {:noreply,
         ...
         |> push_patch(
           to:
             "/go/admin/pipelines/#{pipeline.name}/edit/stages/#{socket.assigns.active_stage.name}/jobs/#{name}/settings"
         )}
  ...
  def handle_event("save_modal", params, socket) do
    case save_modal_result(params, socket) do
      ...
      {:error, {:circular_dependency, path}} ->
        ...
        {:noreply,
         assign(socket, :flash_info, "Error: Circular dependency detected (#{path_str})")}

      {:error, {:missing_pipeline, name}} ->
        {:noreply,
         assign(socket, :flash_info, "Error: Referenced pipeline '#{name}' does not exist")}

      {:error, _reason} ->
        {:noreply,
         assign(socket, :flash_info, "Error saving configuration: invalid values provided.")}
    end
  end
  ...
  defp reload_pipeline_and_stage(socket) do
    ...
    active_stage =
      Enum.find(pipeline.stages || [], &(&1.name == socket.assigns.active_stage.name))

    active_job =
      if active_stage,
        do: Enum.find(active_stage.jobs || [], &(&1.name == socket.assigns.active_job.name))
