        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-amber-700">
            <%= for {agent, i} <- Enum.with_index(@low_agents) do %>
              <%= if i < 3 do %>
                <a href={"/agents/#{agent.uuid}/job_run_history"} class="font-mono text-amber-800 hover:underline">
                  {agent.hostname}
                </a>
            ...
            <%= if length(@low_agents) > 3 do %>
              <span class="text-amber-600">
                ...
                <a href="/agents" class="font-semibold text-amber-800 hover:underline">view all agents</a>
              </span>
            <% end %>
          </span>
        </div>
