describe("HTTP Test Agent Pipeline E2E Test", () => {
  beforeEach(() => {
    ...
    cy.request({ method: "POST", url: "/api/test/start_http_agents", body: { count: "1" }, failOnStatusCode: false });
    cy.visit("/pipelines");
  ...
  it("registers http agent, triggers the demo pipeline, and streams logs", () => {
    ...
    cy.get("body").then(($body) => {
      if (!$body.text().includes("demo") || !$body.text().includes("http-test-agent")) {
