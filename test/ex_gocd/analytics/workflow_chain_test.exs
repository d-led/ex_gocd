defmodule ExGoCD.Analytics.WorkflowChainTest do
  ...
  setup do
    ...
    Pipelines.create_material_for_pipeline(lib, %{type: "git", url: "https://github.com/example/lib.git", name: "lib"})

    {:ok, comp_a} = Pipelines.create_pipeline(%{name: "component-a", group: "test-group"})
    Pipelines.create_material_for_pipeline(comp_a, %{type: "git", url: "https://github.com/example/a.git", name: "a"})
    Pipelines.create_material_for_pipeline(comp_a, %{type: "dependency", url: "upstream-lib", name: "upstream-lib"})

    {:ok, comp_b} = Pipelines.create_pipeline(%{name: "component-b", group: "test-group"})
    Pipelines.create_material_for_pipeline(comp_b, %{type: "git", url: "https://github.com/example/b.git", name: "b"})
    Pipelines.create_material_for_pipeline(comp_b, %{type: "dependency", url: "upstream-lib", name: "upstream-lib"})

    {:ok, integration} = Pipelines.create_pipeline(%{name: "integration-pipeline", group: "test-group"})
    Pipelines.create_material_for_pipeline(integration, %{type: "dependency", url: "component-a", name: "component-a"})
    Pipelines.create_material_for_pipeline(integration, %{type: "dependency", url: "component-b", name: "component-b"})

    {:ok, deploy} = Pipelines.create_pipeline(%{name: "deploy-staging", group: "test-group"})
    Pipelines.create_material_for_pipeline(deploy, %{type: "dependency", url: "integration-pipeline", name: "int"})

    pipeline_names = ["deploy-staging", "integration-pipeline", "component-b", "component-a", "upstream-lib"]
