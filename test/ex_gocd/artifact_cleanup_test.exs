defmodule ExGoCD.ArtifactCleanupTest do
  ...
  describe "directory size calculation" do
    test "correctly sums sizes of nested files", %{artifacts_dir: artifacts_dir} do
      ...
      File.write!(Path.join(artifacts_dir, "file1.txt"), "12345")            # 5 bytes
      File.write!(Path.join([artifacts_dir, "a", "file2.txt"]), "1234567890") # 10 bytes
      File.write!(Path.join([artifacts_dir, "a", "b", "file3.txt"]), "12345") # 5 bytes
  ...
  describe "artifact cleanup purging" do
    test "purges older runs but preserves the latest run and never_cleanup stages",
      ...
      _stage_config = Repo.insert!(%Stage{} |> Stage.changeset(%{
        name: "build",
      ...
      _si2 = insert_stage_instance(pi2.id, "build", 1, false, created_offset_seconds: -50, completed_offset_seconds: -40)
      ...
      refute File.exists?(Path.join([artifacts_dir, "clean-pipe", "1", "build", "1", "job", "data1.txt"]))

      assert Repo.get!(StageInstance, si3.id).artifacts_deleted == false
      assert File.exists?(Path.join([artifacts_dir, "clean-pipe", "3", "build", "1", "job", "data3.txt"]))
    end
    ...
    test "respects never_cleanup_artifacts stage configuration", %{artifacts_dir: artifacts_dir} do
      ...
      assert File.exists?(Path.join([artifacts_dir, "no-clean-pipe", "1", "build", "1", "job", "data.txt"]))
    end
  end
  ...
  defp write_dummy_artifact(artifacts_dir, pipeline, counter, stage, stage_counter, filename) do
    path = Path.join([artifacts_dir, pipeline, to_string(counter), stage, to_string(stage_counter), "job", filename])
