defmodule ExGoCDWeb.API.Admin.PipelineGroupPermissionControllerTest do
  ...
  setup do
    {:ok, user} = Accounts.create_user(%{username: "delegated-admin-#{System.unique_integer()}", display_name: "DA", roles: ["admin"]})
    %{user: user}
  end

  test "grants and lists pipeline group permissions", %{conn: conn, user: user} do
    conn = post(conn, "/api/admin/pipeline_group_permissions", %{user_id: "#{user.id}", pipeline_group: "Build", role: "operator"})
    assert conn.status == 201
    assert %{"data" => %{"role" => "operator", "pipeline_group" => "Build"}} = json_response(conn, 201)
  ...
  test "revokes pipeline group permissions", %{conn: conn, user: user} do
    ...
    conn = delete(conn, "/api/admin/pipeline_group_permissions?user_id=#{user.id}&pipeline_group=Deploy")
    assert conn.status == 200
  ...
  test "rejects invalid role", %{conn: conn, user: user} do
    conn = post(conn, "/api/admin/pipeline_group_permissions", %{user_id: "#{user.id}", pipeline_group: "Build", role: "superadmin"})
