...
ensure_material = fn pipeline, type, url ->
  ...
  mat =
    case Repo.one(from(m in Material, where: m.type == ^type and m.url == ^url, limit: 1)) do
      nil ->
        mat =
          Repo.insert!(%Material{} |> Material.changeset(%{type: type, url: url, branch: branch}))
...
unless Repo.get_by(Pipeline, name: "ci") do
  ...
  IO.puts(
    "Seeded pipeline: ci (stage: build, jobs: unit [no resources], integration [linux,docker])"
  )
else
...
unless Repo.get_by(Pipeline, name: "ex_gocd") do
  ...
  # test job: deps → compile → test (--warnings-as-errors)
  for {cmd, args} <- [
        {"mix", ["deps.get"]},
...
unless Repo.get_by(Pipeline, name: "upstream-lib") do
  Seeds.FanInOut.create_pipeline("upstream-lib", "demo", "build", "compile", "built lib v1.0", [
...
unless Repo.get_by(Pipeline, name: "component-a") do
  ...
  Seeds.FanInOut.create_pipeline(
    "component-a",
    "demo",
    "build",
    "test",
